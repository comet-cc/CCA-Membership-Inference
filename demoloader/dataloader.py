...
from models.mobilenet import create_model as mobilenet_create_model
...
from .tinyimage import TinyImageNetDataset
class CNN(nn.Module):
...
def prepare_dataset(dataset_name, model="resnet18", pretrained=False, path="root", rate=1):


    num_classes, dataset, target_model, shadow_model, pretrained_model = get_model_dataset(dataset_name, attr="attr", root="root", model=model, pretrained=pretrained, path=path)
    length = len(dataset)
    each_length_target = length//4
    each_length_shadow = length//(4*rate)
    print(f"each_length_target = {each_length_target} and each_length_shadow = {each_length_shadow}")
    target_train, target_test, shadow_train, shadow_test, _ = torch.utils.data.random_split(dataset, [each_length_target, each_length_target, each_length_shadow, each_length_shadow, len(dataset)-(each_length_target*2)-(each_length_shadow*2)])
...
def get_model_dataset(dataset_name, attr, root, model="resnet18", pretrained=False, path="root"):
    if dataset_name.lower() == "utkfacerace":
        ...
        if isinstance(attr, list):
            ...
            if attr == "attr":
#                num_classes = 4
                attr_list  = [[18, 20, 21, 31, 39]]
                num_classes = 2 ** len(attr_list[0])
                #attr_list = [[31, 39]]
            else:
                raise ValueError("Target type \"{}\" is not recognized.".format(attr))
...
#                num_classes = 4
    ...
    elif dataset_name.lower() == 'imagenet-64':
        num_classes = 1000
        traindir = os.path.join(root, 'imagenet', 'imagenet', 'train')
        testdir = os.path.join(root, 'imagenet', 'imagenet', 'test')
        ...
        num_classes = 200
        traindir = os.path.join(root, 'tiny-imagenet-200')
        #testdir = os.path.join(root, 'tiny-imagenet-200')
        #train = TinyImageNet(traindir, split='train')
        #test = TinyImageNet(testdir, split='val')
        train_transform = transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),
        ])
        train = TinyImageNetDataset(root_dir=traindir, mode='train', transform=train_transform)
        test = TinyImageNetDataset(root_dir=traindir, mode='val', transform=train_transform)
        dataset = train + test
...
#transform=None, download=False, max_samples=None):
    ...
    if model == "cnn":
        ...
        if isinstance(num_classes, int):
            ...
            pretrained_model = mobilenet_create_model(model, num_classes=num_classes, pretrained=False)
        else:
            ...
            pretrained_model = mobilenet_create_model(model, num_classes=num_classes[0], pretrained=False)
    elif model.startswith("inceptionv3") :
        if isinstance(num_classes, int):
            target_model = inception_v3(num_classes=num_classes)
            shadow_model = inception_v3(num_classes=num_classes)
            pretrained_model = inception_v3(init_weights=True,num_classes=num_classes)
        else:
            target_model = inception_v3(num_classes=num_classes[0])
            shadow_model = inception_v3(num_classes=num_classes[0])
            pretrained_model = inception_v3(init_weights=True,num_classes=num_classes[0])
    else:
