...
class TinyImageNetPaths:
  def __init__(self, root_dir, download=False):
    if download:
      download_and_unzip('https://image-net.org/data/tiny-imagenet-200.zip',
                         root_dir)
...
class TinyImageNetDataset(Dataset):
  ...
  def __getitem__(self, idx):
    ...
    # Apply the transform (which should include ToTensor if specified)
    if self.transform:
    ...
    # Return image tensor and label (or dummy label for test mode)
    return (img, lbl)
