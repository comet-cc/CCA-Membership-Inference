...
def convert_pytorch_to_tflite(model, dummy_input_shape, tflite_model_path, PATH):
    """
    Converts a PyTorch model to TensorFlow Lite format.
    """
    # Step 1: Export the PyTorch model to ONNX
    onnx_model_path = osp.join(PATH, "temp_model.onnx")
    ...
    tf_model_path = osp.join(PATH, "temp_model.pb")
    ...
    # Check if tf_model_path is a directory and delete accordingly
    if os.path.isdir(tf_model_path):
        shutil.rmtree(tf_model_path)  # Delete the directory
    elif os.path.isfile(tf_model_path):
        os.remove(tf_model_path)  # Delete the file if it is a file

    print(f"Model successfully converted to {tflite_model_path}")

def convert_pytorch_to_onnx(model, dummy_input_shape, onnx_model_path):
    """
    Converts a PyTorch model to TensorFlow Lite format.
...
def train_model(PATH, device, train_set, test_set, model, args):
    ...
    TF_FILE_PATH = osp.join(PATH, "target.tflite")
    convert_pytorch_to_tflite(model.net.eval().cpu(), dummy_input_shape=(1, 3, 64, 64), tflite_model_path=TF_FILE_PATH, PATH=PATH)

#    ONNX_FILE_PATH = osp.join(PATH, "target.onnx")
 #   convert_pytorch_to_onnx(model.net.eval().cpu(), dummy_input_shape=(1, 3, 64, 64), onnx_model_path=ONNX_FILE_PATH)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--rate', type=int, default=1, metavar='N',
                        help='Change the rate between size of shadow and training datasets (default: 1)')
    ...
    num_classes, target_train, target_test, shadow_train, shadow_test, target_model, shadow_model, pretrained_model = prepare_dataset(
        args.dataset.lower(), args.model_arch, pretrained=args.pretrained, path=current_directory, rate=args.rate
    )
...
#    )
    test_meminf_full(
        args.out_path, args.out_path, args.out_path,
        device, num_classes, target_train, target_test, shadow_train, shadow_test, 
        target_model, shadow_model, args
    )
